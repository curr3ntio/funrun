...
A second, independent page: two striped worms tangled on a paper-textured
...
Serve the folder and open `slug.html` (ES modules need `http://`, not `file://`).
...
drag-orbit and auto-orbit toggles. Defaults live in `CONFIG` inside `slug.html`.
